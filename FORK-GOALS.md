...
- **UI and accessibility:** the interface should be comfortable to use for a whole session, on a phone or tablet as readily as a desktop. Legibility, touch targets, colour contrast, and keyboard and screen-reader support are treated as features, not polish.
- **External integrations:** finished games should be able to leave the app. Recording results in trackers such as Marvel Champions Tracker and BGStats — by export, or by direct integration where an API allows — is a supported goal rather than a manual chore.
